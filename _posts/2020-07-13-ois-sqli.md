...
Great this looks like a great start. In cases of black box testing or when the application disables any errors be shown, we will need to verify that we can succesfully inject additional queries in the database a different way. As such we can use a payload that will cause the database to sleep for a certain amount of time to indeed confirm that injection was possible such as:
